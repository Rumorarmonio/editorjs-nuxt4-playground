...
- Активный scope по `SPEC.md`: первый простой custom block без nested editors.
- Для первого custom block выбран `Notice`, чтобы проверить полный custom block lifecycle на простых plain fields без раннего перехода к media workflow, rich fields или nested Editor.js.
...
- Отдельный visible usage harness для plain fields не добавлен; первое runtime-использование и ручная проверка запланированы в рамках первого custom block.
...
- Проект переведён к этапу первого простого custom block: активным блоком для реализации выбран `Notice`, следующий крупный этап после него — первый nested editor block / single-purpose rich field.
- Реализован первый вертикальный срез `Notice`: добавлены shared type/normalization, регистрация block type, Editor.js tool на plain fields (`title`, `text`, `type`), renderer-отображение, draft guard и demo block в `content/default-page.json`; `npm run check` проходит.
...
- Runtime-удобство plain fields теперь можно проверить на реальном custom block `Notice`; отдельный demo harness на этапе plain fields не добавлялся.
- Для закрытия текущего этапа нужна ручная browser-проверка полного цикла `Notice`: create -> edit -> save -> reload -> render, а также export/reset draft.
...
Следующий шаг: вручную проверить полный цикл `Notice` custom block в браузере: create -> edit -> save -> reload -> render, export JSON и reset draft.
