...
- Активный scope по `SPEC.md`: sidebar navigation из JSON после завершения media gallery / slider block.
...
- Проект переведён к следующему крупному этапу по `SPEC.md`: sidebar navigation из JSON.
...
- Проект переведён к этапу sidebar navigation из JSON: активный план сфокусирован на flat navigation, построенной по `AnchorTune` и `LabelTune`.
...
Следующий шаг: проанализировать текущие tune helpers, renderer anchor logic и preview layout перед реализацией flat sidebar navigation из JSON.
