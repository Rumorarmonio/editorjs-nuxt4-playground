...
- Активный scope по `SPEC.md`: composite blocks / `TwoColumns` после завершения первого nested editor scenario и reusable rich fields завершён; следующий крупный этап — custom inline tools.
...
- Этап composite blocks / `TwoColumns` реализован как первый custom block с двумя независимыми nested editor containers; `npm run check` проходит.
...
- Реализован этап composite blocks / `TwoColumns`: добавлены shared-типы `TwoColumnsBlockData` / `TwoColumnsContentData`, normalizers/guards, registry entry, `TwoColumnsTool`, column nested tools, renderer, стили и demo block в `content/default-page.json`; `npm run check` проходит.
...
- Ручная browser-проверка `TwoColumns` ещё нужна: create/edit/save/reload/render, delete/move block, export JSON и reset draft.
...
Следующий шаг: вручную проверить `TwoColumns` в браузере: create/edit/save/reload/render, delete/move block, export JSON и reset draft.
