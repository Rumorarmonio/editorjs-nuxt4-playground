...
- Базовая версия проекта завершена; выбран новый активный scope по `SPEC.md`: базовые Block Tunes (`AnchorTune`, `SpacingTune`, `LabelTune`).
...
- Реализована основа этапа базовых Block Tunes: добавлены `AnchorTune`, `SpacingTune` и `LabelTune`, shared-нормализация/проверка tune data, renderer-обработка `anchor` / `spacing` / `label` и демонстрационные tune examples в `content/default-page.json`.
...
- Базовые Block Tunes реализованы в коде; остаётся ручная browser-проверка save/load, preview/render, reset draft и export JSON на блоках с tune data.
...
Следующий шаг: вручную проверить workflow базовых Block Tunes в браузере и после успешной проверки закрыть активный этап в `PLAN.md`.
