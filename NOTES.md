...
- Следующий этап детализирован как `SectionIntro`: один простой custom block с plain title и одним nested rich paragraph field.
...
- Reusable `RichParagraphField` / `RichHeaderField` остаются отдельным следующим этапом после проверки первого nested editor lifecycle.
...
- Для активного этапа выбран формат: первый nested editor block одновременно проверяет первый single-purpose rich field.
- Минимальный helper для create/destroy nested Editor.js instance выбран и реализован как single-purpose `createNestedParagraphEditor`; полноценная reusable rich field abstraction остаётся вне текущего этапа.
- Ручная browser-проверка полного `SectionIntro` lifecycle в этой сессии ещё не выполнялась: нужно пройти create/edit/save/reload/render/destroy, export JSON и reset draft.
...
Следующий шаг: вручную проверить `SectionIntro` в браузере: создание из toolbox, inline formatting во вложенном paragraph field, save/reload/render, export JSON и reset draft.
