...
- Проект переведён к этапу Клавиатурная навигация и accessibility polish: первый фокус — keyboard-аудит shell controls, custom tools, field wrappers, media/composite scenarios, focus states и отсутствие критичных focus traps без изменения content JSON schema.
- Активный этап Некритичные улучшения начат с language switcher improvement: язык теперь выбирается через dropdown по аналогии с темой, поддерживает preference `system | ru | en | es`, автоматически определяется по browser/OS language и включает испанский интерфейс.
- Для активного этапа Некритичные улучшения реализуется временный demo-переключатель sidebar navigation на preview-странице: `headings` строит вложенную навигацию из JSON по `Header.level`, `labels` сохраняет старый режим по `AnchorTune` / `LabelTune`. В будущем такое переключение должно быть page-level настройкой каждой контентной страницы, а не глобальным UI state текущего preview.
- Для активного этапа Некритичные улучшения добавлен `AnimationTune`: блоки могут хранить простую reveal-анимацию в `block.tunes.animation`, а проигрывание выполняется только в renderer через `IntersectionObserver` и CSS с учётом `prefers-reduced-motion`.
- Для активного этапа Некритичные улучшения добавлен scoped `EmbedDisplayTune`: tune подключается только к `embed` tool, хранит режим `inline | fancybox` в `block.tunes.embedDisplay`, а renderer в Fancybox-режиме показывает iframe-превью на странице, блокирует прямой клик по нему и открывает видео через Fancybox overlay.
...
- Глобальный admin skin Editor.js должен оставаться рядом с `editor/admin`, а не растворяться в app-level стилях, чтобы будущий admin frontend мог импортировать его отдельно от Nuxt/Vue shell.
...
- `CTA/Button` выбран как custom typed block, а не сторонний plugin: данные хранят `label`, safe `url`, optional `description`, `variant` и явный `target`.
- Code block выбран как custom typed `codeSnippet` с language field, plain code textarea и renderer-side `<pre><code>` без syntax highlighting dependency на текущем этапе.
- `Raw HTML` использует готовый editor-side tool `@editorjs/raw` под ключом `rawHtml`; shared data contract хранит только `{ html }` без caption.
- `Raw HTML` renderer имеет явный code-level режим `safe | unsafe`: по умолчанию включён `unsafe`, чтобы Raw HTML работал как trusted admin-only escape hatch с пользовательскими styles/scripts; `safe` можно включить в коде, тогда renderer применяет allowlist sanitizer и удаляет scripts, iframes, event handlers, inline styles и unsafe attributes.
- Перед повторным render/unmount `Raw HTML` renderer отправляет `editor:raw-html-cleanup` на контейнер и очищает DOM; scripts, которые создают внешние listeners/timers/widgets, должны подписываться на это событие и убирать свои side effects самостоятельно.
- CTA event action хранится как typed JSON (`actionType: event`, `eventName`), renderer dispatches `editor:cta-action`, а demo-событие `open-demo-modal` показывает локальное модальное окно без сохранения функций в content JSON.
- Syntax highlighting для `Code snippet` выполняется только в renderer через `highlight.js` core с регистрацией ограниченного набора языков; editor tool остаётся plain textarea.
- `@editorjs/link` отложен до появления backend/server route для metadata fetching; frontend-only preview не должен имитировать production link preview без стабильного source of truth.
- `@editorjs/warning` не добавляется, потому что текущий `NoticeTool` уже покрывает info/success/warning сценарии и имеет renderer/validation/i18n contract.
...
- Keyboard/accessibility polish должен улучшать основные editor-related сценарии без обещания полного keyboard-only покрытия внутреннего UI Editor.js.
- Для accessibility polish приоритетны нативные интерактивные элементы, видимые `focus-visible` состояния, предсказуемый tab-order и отсутствие критичных focus traps между shell controls, Editor.js и nested editors.
...
- Проект переведён к этапу Клавиатурная навигация и accessibility polish: активный план сфокусирован на keyboard-аудите editor/preview shell, custom fields/tools, `MediaGallery`, composite/nested scenarios, focus states и минимальных accessibility-атрибутах для кастомного UI.
- Первый accessibility-fix активного этапа внесён и уточнён: Editor.js больше не автофокусируется при инициализации, plain field wrappers явно отделены от inherited contenteditable-контекста, keyboard events от custom controls не всплывают в Editor.js/document handlers, верхние controls editor page получили ранний Tab-boundary на document capture, toggle track стал кликабельным через label-wrapper, добавлены pointer/focus-visible states для toggle и media card buttons, enabled media card buttons явно отделены от contenteditable.
- Post-review fixes первого accessibility-fix внесены: `contentEditable=false` теперь применяется только к plain interactive controls, rich/nested editor wrappers не отделяются от contenteditable-контекста, radio group вернулась к нативному tab-order. Header Tab-boundary оставлен на `stopImmediatePropagation`, потому что более мягкий `stopPropagation` вернул перехват `Tab` обработчиками Editor.js после возврата фокуса из editor surface.
- Table Tool keyboard patch добавлен как изолированный DOM-helper без форка `@editorjs/table`: ячейки сохраняют дефолтные `Tab` / `Shift+Tab` и `Enter`, а внешние controls `.tc-add-column`, `.tc-add-row`, `.tc-toolbox__toggler` и `.tc-popover__item` получают tab stops, `role="button"`, `aria-label`, `Enter` / `Space` activation и `focus-visible` styles.
- Floating Editor.js toolbar keyboard patch добавлен как изолированный DOM-helper без форка Editor.js: текущие `.ce-toolbar__plus`, `.ce-toolbar__settings-btn` и `.ce-popover-item` получают tab stops, `role="button"`, `aria-label`, `Enter` / `Space` activation и `focus-visible` styles. Для `.ce-toolbar__settings-btn` клавиатурная активация имитирует `mousedown`, потому что Editor.js открывает block tunes именно на этом событии, а не на `click`. Последовательный обход toolbar controls каждого блока не реализуется, потому что Editor.js использует один floating toolbar для активного блока.
- Shortcuts `/` и `Ctrl + /` оставлены в дефолтной зоне ответственности Editor.js: они надёжны для caret внутри стандартных contenteditable-блоков и nested editors, но не форсируются из plain custom controls, чтобы ввод `/` в полях не открывал toolbox/tunes внешнего редактора.
- Завершён этап Клавиатурная навигация и accessibility polish: ручная проверка подтвердила исправления tab-order верхней панели, focus/activation для toggles/selects/enabled buttons, Table Tool controls и floating Editor.js toolbar. `npm run check` и `npm run build` проходят; `npm run check` сохраняет существующие предупреждения `vue/no-v-html`. Проект переведён к этапу Некритичные улучшения.
- Этап Некритичные улучшения временно отложен после серии optional improvements: language switcher, heading-based sidebar navigation, block reveal animations, embed Fancybox display, редактирование URL существующего embed-блока, декомпозиция renderer components и вынос admin editor skin в глобальный stylesheet. После реализации следующего этапа к нему можно вернуться, потому что новые улучшения появляются постоянно.
- Первое optional improvement этапа Некритичные улучшения реализовано: добавлен locale preference `system | ru | en | es`, browser/OS language resolver с fallback-группами `ru` для русского/постсоветского языкового окружения, `es` для испанского и языков Испании, `en` для остальных, dropdown UI на editor/preview страницах и испанские app/editor словари. `npm run check` и `npm run build` проходят.
- Post-review fixes language switcher внесены: auto language resolver теперь учитывает region subtag после script subtag, а dropdown handlers проверяют выбранное значение через `isLocalePreference` перед записью в состояние.
- Heading-based sidebar navigation реализована как второе optional improvement: добавлены auto anchors для header-блоков без `AnchorTune`, tree builder по `Header.level`, рекурсивный renderer sidebar и временный preview-переключатель `По заголовкам` / `По якорям и меткам`. `content/default-page.json` расширен дополнительными заголовками `Table`, `Embed YouTube video`, `Embed Vimeo video`, `Media blocks`, `Utilities` и другими секциями для демонстрации вложенности.
- Post-review fixes heading sidebar внесены: label navigation теперь использует те же rendered anchor ids, что и renderer, переключатель режима остаётся доступным, если хотя бы один режим содержит пункты, а sticky positioning оставлен только на внешнем sidebar.
- Block reveal animations реализованы как третье optional improvement: добавлен `AnimationTune` с вариантами `none`, `fade-up`, `fade-left`, `fade-right`, renderer-side `IntersectionObserver`, CSS reveal states и demo values в `content/default-page.json`. Анимации не проигрываются в editor canvas и не добавляют внешних зависимостей.
- Embed Fancybox display реализован как четвёртое optional improvement: `EmbedDisplayTune` подключён только к `embed`, общий whitelist embed-сервисов расширен Rutube, VK Video и Twitch, preview renderer поддерживает inline iframe и Fancybox iframe modes, а `content/default-page.json` содержит inline/Fancybox examples для YouTube/Vimeo/Coub/Rutube/VK Video. Для Twitch iframe URL нормализуется с `parent` текущего hostname при рендере.
- Embed URL editing реализован как точечное UX-улучшение существующего `ManualEmbedTool`: созданный embed-блок теперь можно перевести из preview в форму редактирования URL, применить новый поддержанный URL без удаления блока и сохранить текущую caption при замене видео.
- Renderer components декомпозированы: `EditorContentRenderer` оставлен как document-level loop с anchors, spacing, animation/Fancybox binding, конкретные block renderer components перенесены в `editor/renderer/components/blocks`, а `EditorSidebarNavigation` поднят в корень `editor/renderer/components`.
- Этап расширения набора контентных блоков и plugins завершён: добавлены shared-типы, normalizers/guards, registry entries, validation, localized editor UI, tools `CtaTool` / `CodeSnippetTool`, готовый `@editorjs/raw` под ключом `rawHtml`, renderer components `EditorCtaBlock` / `EditorCodeSnippetBlock` / `EditorRawHtmlBlock`, theme-aware admin/renderer styles и demo blocks в `content/default-page.json`. Новые blocks подключены только к основному editor; nested editor whitelist'ы не расширялись. `npm run check` проходит с существующими и новыми ожидаемыми предупреждениями `vue/no-v-html`; `npm run build` проходит.
- Post-review fixes этапа расширения blocks/plugins внесены: CTA renderer учитывает Nuxt `baseURL` для root-relative links, protocol-relative `//` URL запрещены в CTA contract, invalid CTA URL показывает отдельную validation error, а `Code snippet` сохраняет крайние пробелы и пустые строки кода.
- Review-fixes для расширения blocks/plugins внесены: Raw HTML sanitizer удаляет запрещённые элементы целиком и нормализует root-relative links через Nuxt `baseURL`; позже editor-side Raw HTML переведён на `@editorjs/raw`, caption удалён, renderer получил safe/unsafe режим с unsafe default, cleanup event перед повторным render/unmount и перестал добавлять декоративный фон; CTA event name больше не переписывается silently, а валидируется как введённый идентификатор; общий CTA renderer только dispatches `editor:cta-action`, а demo modal перенесён в preview shell; `CtaTool` скрывает нерелевантные поля для link/event action types. `npm run check` и `npm run build` проходят.
- Ручная проверка подтвердила работу `CTA/Button` и `Code snippet`; `Raw HTML` после перехода на `@editorjs/raw`, CTA event action и syntax highlighting реализованы и требуют ближайшего browser smoke-check.
- CTA упрощён до самостоятельной кнопки: renderer выводит только `<a>` для link action или `<button>` для event action без description и фонового контейнера, `CtaTool` больше не сохраняет description, а `cta` разрешён во вложенных rich editors (`SectionIntro`, `TwoColumns`, media card descriptions). `content/default-page.json` содержит nested CTA examples; `npm run check` проходит.
- CTA link action теперь рендерит root-relative ссылки текущей вкладки через `NuxtLink`, чтобы переходы внутри приложения шли без полной перезагрузки страницы; внешние ссылки, `mailto`, anchors и `newTab` остаются обычным `<a>`.
...
- Keyboard-polish может конфликтовать с внутренними focus/selection сценариями Editor.js и nested editors, поэтому изменения нужно вносить точечно и проверять вручную.
