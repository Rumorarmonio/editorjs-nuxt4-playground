...
1. Небольшие polished interactions и UX-улучшения, которые не требуют нового крупного content contract.
2. Поздние tunes и renderer/admin improvements, если они остаются точечными.
3. Media/navigation improvements, которые не ломают текущие save/load/render/import/export/reset сценарии.
4. Отложенные smoke-check проверки уже реализованных optional improvements.

Вне scope этапа:

- новые крупные schema-changing этапы без отдельного решения;
- полноценная CMS/page-management админка;
- backend/API/upload workflow;
- широкий UI-kit rewrite вместо точечных улучшений.
...
Конкретный сервис для первой версии этапа: **DummyJSON**. Он подходит для этой задачи, потому что уже предоставляет `products`, `posts` и `recipes`, а также серверную сортировку/пагинацию и ограниченные фильтры на уровне этих сущностей.

В scope входят:

1. Поддержка нескольких заранее известных сущностей, достаточных для демонстрации поведения блока: `products`, `posts`, `recipes`.
2. Базовые варианты отображения секции: `grid`, `slider`, `list`.
3. Ограниченные server-like параметры выборки, которые реально поддерживает выбранный сервис: `limit`, `skip`, `sortBy`, `order`, category/tag/meal-type presets.
4. Пагинация и дозагрузка карточек через `showLoadMore` и `loadMoreStep` для grid/list-сценариев.
5. Отдельный toggle для `showViewAllButton`, который формирует переход на страницу/маршрут сущности и может нести текущие query-параметры сортировки и фильтрации.
6. Опциональные visitor controls для показа публичных селектов сортировки/фильтрации только по заранее разрешённым сценариям.
7. Декларативный контракт блока без вложенных editor runtime-контролов и без произвольного query builder UI.

Вне scope этапа:

- собственные domain entities и полноценный backend API;
- произвольный фильтр-конструктор для любых полей сущностей;
- ручная сортировка массивов карточек на клиенте как основной сценарий;
...
Итог: source SVG перенесены из `public` в `editor/assets/icons`, generated sprite создаётся в `public/icons/sprite.svg`, generated `IconName` / `iconNames` и shared
helpers добавлены в `editor/shared/icons`, CTA schema расширена `contentMode`, `leftIcon`, `rightIcon` и `icon`, renderer показывает left/right/both/icon-only variants,
а `CtaTool` получил searchable icon selects на базе `choices.js` с preview иконок.

Проверки: `npm run generate-svg-sprite`, `npm run check` и `npm run build` выполнялись в ходе этапа; ручной browser smoke-check CTA icon scenarios остаётся полезной
проверкой перед финальным коммитом.
...
3. Renderer-компоненты: атомарный переиспользуемый `Accordion` с module styles и именованными слотами `head` / `body`, плюс `AccordionGroup`, который адаптирует block data, управляет state группы и использует 0.3s height-анимацию через измерение содержимого.
...
- Атомарный renderer `Accordion` остаётся переиспользуемым компонентом со слотами `head` и `body`, без прямой зависимости от `accordionGroup` schema.
